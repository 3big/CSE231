...
namespace llvm{
    ...
    public:
        ...
        std::map<unsigned, std::set<unsigned >> mayPointTo_defs ={};
...
[pointer's DFA ID]->([pointee 1's DFA ID][slash][pointee 2's DFA ID][slash] ... [pointee m's DFA ID][slash])*/
            ...
            //Determine which pointers reference an object on the heap
            ...
            for(auto pointer = mayPointTo_defs.begin(), end = mayPointTo_defs.end(); pointer != end; pointer++ ) {
                ...
                for (auto pointeeID: pointer->second) {
                    ...
                    if (was_alloca[pointeeID]){//alloca
                        pointee_prefix = 'M';//should point to the location of a heap-allocated object
                    }
...
/////////////////////*join incoming edges*/////////////////////////////
         ...
         * i is the index of the IR instruction that allocates this memory object. For example, suppose that the following IR instruction's index is 10*/
            ...
            else if (instr_opcode == 32) {//getelementptr
            ...
            else if (instr_opcode == 55) { //select
            ...
            else{ //any other LLVM IR instruction
                //out = in
            }
            //final reaching info
